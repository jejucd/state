...
Agent version, 감지된 Java version, 실행 중인 service, 현재 사용 중인 port 같은 live fact는 데이터베이스에 저장합니다.
...
Domain 또는 app RBAC 파일이 없으면 parent access를 그대로 상속합니다. 한 user가 여러 role에 매칭되면 가장 높은 role을 사용합니다.
...
JejuCD state는 raw global Nginx 설정이 아니라 Nginx route intent를 저장합니다.
...
Agent는 노드 로컬에 다음과 같은 설정을 렌더링합니다.
